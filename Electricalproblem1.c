...
int main()
{
  ...
  Vrms=Vm/sqrt(2);
  scanf("%lf",&Vrms);
  printf("the rms voltage is :%lf\n",Vrms);
  Irms =Im/sqrt(2);
  scanf("%lf",&Irms);
  printf("the rms current is :%f\n",Irms);
  Z=Vm/Im;
  scanf("%d",&Z);
  ...
  R=Z*pf;
  scanf("%lf",&R);
  printf("the value of resistance is:%lf\n",R);
  P=Vm*(Im/2)*pf;
  scanf("%lf",&P);
  printf("the power consumed is :%lf\n",P);
  S=sin(phaseangle);
  scanf("%lf",&S);
  ...
  L=w/Xl;
  scanf("%lf",&L);
  printf("the value inductance is :%lf\n",L);
  return 0;
}